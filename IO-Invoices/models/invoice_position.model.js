...
Invoice_position.create = (newInvoice_position, result) => {
  sql.query(
    "INSERT INTO invoice_position SET ?",
    newInvoice_position,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
...
Invoice_position.updateById = (id, invoice_position, result) => {
  sql.query(
    "SELECT * FROM invoice_position WHERE id='" + id + "'",
    (err, res1) => {
      sql.query(
        "UPDATE invoice_position SET quantity=?, total=? WHERE id=?",
        [
          invoice_position.quantity,
          invoice_position.quantity * res1[0].unit_price,
          id,
        ],
        (err, res) => {
          if (err) {
          ...
          console.log("updated invoice_position: ", {
